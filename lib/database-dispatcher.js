...
class DatabaseDispatcher {
	...
	static _setConfig() {
		...
		if(typeof this.config !== 'object' || Array.isArray(this.config))
			throw new DatabaseDispatcherError('Invalid config file, should be an object', DatabaseDispatcherError.codes.INVALID_CONFIG_FILE);
	}
	...
	static _getConfigFromEnvVars(key) {
		...
		if(!dbConfig.host || !dbConfig.type)
			return;
	...
	static _getDatabaseByConfig(dbConfig) {
		...
		if(!this.databases[dbConfig.host])
			this.databases[dbConfig.host] = this._getDBDriver(dbConfig);

		return this.databases[dbConfig.host];
	}
